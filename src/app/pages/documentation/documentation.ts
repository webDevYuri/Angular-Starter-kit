...
export class DocumentationComponent implements AfterViewInit {
  ...
  coreFolders = [
    { path: 'core/constants/',    desc: 'Fixed values used across the application — API URLs, regex patterns, and app-wide configuration.',                            code: `core/constants/\n├── api.constant.ts        # API base URLs and endpoint paths\n├── regex.constant.ts      # Reusable regex patterns\n└── app-config.constant.ts # Pagination size, date formats, etc.` },
    ...
    { path: 'core/directives/',   desc: 'Custom attribute directives that add reusable behavior to DOM elements without creating full components.',                    code: `core/directives/\n├── click-outside.directive.ts  # Detect clicks outside an element\n├── auto-focus.directive.ts     # Auto-focus input on render\n└── debounce-click.directive.ts # Prevent rapid double-clicks` },
    { path: 'core/enums/',        desc: 'Named constant sets that make code more readable and type-safe than raw strings or numbers.',                                 code: `core/enums/\n├── user-role.enum.ts      # Admin, Editor, Viewer\n├── status.enum.ts         # Active, Inactive, Pending\n└── http-status.enum.ts    # Ok, NotFound, ServerError` },
    ...
    { path: 'core/interfaces/',   desc: 'TypeScript interfaces that define the shape of objects and API responses for type safety across the app.',                    code: `core/interfaces/\n├── user.interface.ts      # IUser, IUserProfile\n├── api-response.interface.ts # IApiResponse<T>, IPaginatedResponse<T>\n└── table.interface.ts     # ITableColumn, ITableConfig` },
    { path: 'core/models/',       desc: 'TypeScript classes with behavior — computed properties, methods, and constructor logic beyond plain shapes.',                  code: `core/models/\n├── toast.model.ts         # Toast class with type, message, duration\n├── confirm.model.ts       # ConfirmOptions with defaults\n└── user.model.ts          # User class with getInitials(), fullName` },
    ...
    { path: 'core/utils/',        desc: 'Pure helper functions with no side effects — formatting, parsing, and data transformation utilities.',                        code: `core/utils/\n├── date.util.ts           # Format dates, calculate differences\n├── string.util.ts         # Capitalize, slugify, truncate\n└── file.util.ts           # File size formatting, extensions` },
    { path: 'core/validators/',   desc: 'Custom form validators for reactive and template-driven forms beyond Angular\'s built-in validators.',                        code: `core/validators/\n├── match.validator.ts     # Confirm password matches password\n├── no-whitespace.validator.ts # Disallow leading/trailing spaces\n└── unique-email.validator.ts  # Async check email availability` }
  ];
